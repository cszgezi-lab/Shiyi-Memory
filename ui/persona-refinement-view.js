...
<details data-review-batches><summary>精修批次管理 · Markdown</summary><div class="sy-actions"><button type="button" data-review-pause>暂停</button><button type="button" data-review-resume>继续未完成</button><button type="button" data-review-apply>应用候选</button><button type="button" data-review-discard>删除整组候选</button></div><div class="sy-actions"><button type="button" data-review-prev>上一页</button><span data-review-page></span><button type="button" data-review-next>下一页</button></div><div class="sy-actions"><button type="button" data-review-select-page>选中本页</button><button type="button" data-review-clear>清空选择</button><button type="button" data-review-delete-selected>删除所选</button></div><p class="sy-help" data-review-selection role="status"></p><div data-review-items></div><details><summary>查看 Markdown 记录</summary><pre class="sy-packet" data-review-markdown></pre></details></details>
...
export function mountPersonaRefinement({panel,app,run,host=globalThis}){
  ...
  let scope='',profilesStamp='',preview=null,dirty=false,itemsStamp='',page=0,planId='';
  const selection=new Set(),pageSize=10;
  const guard=()=>{const expected=JSON.stringify([app.state.core?.scope,app.state.dynamicPersona?.refinement?.manualPlan?.id]);return ()=>{if(JSON.stringify([app.state.core?.scope,app.state.dynamicPersona?.refinement?.manualPlan?.id])!==expected)throw new Error('聊天或精修计划已切换，操作未执行');};};
  ...
  bind('[data-review-apply]',async()=>{const check=guard();if(await host.confirm?.('应用已保存的精修候选？正式档案会保存旧版；已删除批次不参与。历史整理建议仍需在人物页确认。')){check();await app.applyDynamicPersonaReview();}});
  bind('[data-review-discard]',async()=>{const check=guard(),applied=app.state.dynamicPersona?.refinement?.manualPlan?.status==='applied';if(await host.confirm?.(applied?'撤回这组已应用精修，恢复精修前档案？后续已有新版本时会拒绝覆盖。':'删除整组精修候选？保留归档，正式档案和主总结不变。')){check();await app.manageDynamicPersonaReview('discard');}});
  ...
  $('[data-review-items]').addEventListener('click',e=>{const button=e.target.closest?.('[data-review-action]');if(!button)return;run(()=>button.dataset.reviewAction==='delete'?deleteSelected([button.dataset.reviewId]):app.manageDynamicPersonaReview(button.dataset.reviewAction,button.dataset.reviewId),{name:'dynamic-persona',button});});
  $('[data-review-items]').addEventListener('change',e=>{const input=e.target.closest?.('[data-review-select]');if(!input||input.disabled)return;input.checked?selection.add(input.dataset.reviewSelect):selection.delete(input.dataset.reviewSelect);renderer.paint(app.state);});
  for(const [key,delta] of [['prev',-1],['next',1]])bind('[data-review-'+key+']',()=>{page=Math.max(0,page+delta);renderer.paint(app.state);});
  bind('[data-review-select-page]',()=>{for(const i of (app.state.dynamicPersona?.refinement?.manualPlan?.items??[]).slice(page*pageSize,(page+1)*pageSize))if(i.status!=='deleted')selection.add(i.id);renderer.paint(app.state);});
  bind('[data-review-clear]',()=>{selection.clear();renderer.paint(app.state);});
  bind('[data-review-delete-selected]',()=>deleteSelected([...selection]));
  const renderer={paint(s){
    const d=s.dynamicPersona??{},nextScope=JSON.stringify(s.core?.scope);
    if(scope!==nextScope){scope=nextScope;profilesStamp=itemsStamp='';dirty=false;page=0;selection.clear();invalidate();$('[data-review-start]').value=1;$('[data-review-size]').value=10;}
    ...
    if(planId!==plan?.id){planId=plan?.id;page=0;selection.clear();}
    const items=plan?.items??[],pages=Math.max(1,Math.ceil(items.length/pageSize));page=Math.min(page,pages-1);
    for(const id of selection)if(!active||!items.some(i=>i.id===id&&i.status!=='deleted'))selection.delete(id);
    $('[data-review-page]').textContent=`第 ${page+1}/${pages} 页 · 共 ${items.length} 批`;
    $('[data-review-prev]').disabled=page===0;$('[data-review-next]').disabled=page===pages-1;
    $('[data-review-selection]').textContent=`已选 ${selection.size} 批（跨页保留）`;
    $('[data-review-clear]').disabled=!selection.size;$('[data-review-delete-selected]').disabled=!active||!selection.size;
    $('[data-review-select-page]').disabled=!active||!items.slice(page*pageSize,(page+1)*pageSize).some(i=>i.status!=='deleted');
    const snapshot=JSON.stringify([plan?.id,plan?.status,state.revision,page,[...selection],items.map(i=>[i.id,i.status]),state.queue?.map(t=>[t.id,t.status,t.attempts])]);if(snapshot===itemsStamp)return;itemsStamp=snapshot;
    ...
    $('[data-review-items]').innerHTML=items.slice(page*pageSize,(page+1)*pageSize).map(item=>{const q=state.queue?.find(t=>t.id===item.id),status=q?.status??item.status;return `<div class="sy-card">${active&&item.status!=='deleted'?`<label><input type="checkbox" data-review-select="${esc(item.id)}" ${selection.has(item.id)?'checked':''}> 选择 #${item.startIndex}–${item.endIndex}</label>`:''}<p>#${item.startIndex}–${item.endIndex} · ${esc(labels[status]??status)}</p><div class="sy-actions">${q?.status==='failed'&&active?`<button type="button" data-review-action="retry" data-review-id="${esc(item.id)}">重试本批</button>`:''}${active&&item.status!=='deleted'?`<button type="button" data-review-action="delete" data-review-id="${esc(item.id)}">删除候选</button>`:''}</div></div>`;}).join('')||'<p>还没有手动精修批次。</p>';
  }};return renderer;
}
