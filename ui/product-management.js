...
export function mountDictionary({panel,app,run,save=input=>app.saveDictionaryEntry(input)}){
  ...
  function paint(s){snapshot=s;const q=$('[data-dictionary-search]').value.trim().toLocaleLowerCase(),entries=(s.dictionary?.entries??[]).filter(e=>`${e.name} ${e.aliases.join(' ')}`.toLocaleLowerCase().includes(q));
