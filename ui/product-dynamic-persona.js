...
export function mountDynamicPersona({panel,app,run,host=globalThis}){
  ...
  function paint(s){
    ...
    for(const p of visible.slice(profilePage*6,profilePage*6+6)){if(p.deleted)continue;if(editing.has(p.id)){cards.push(editing.get(p.id));continue;}
      ...
      card.innerHTML=`<h4>${esc(p.name)}</h4><p class="sy-help">${p.locked?'已锁定 · ':''}依据至 #${p.through} · ${p.bindings?.length?'已关联原书，实际替换见本轮注入':'尚未接管原书 · 仅补充'}</p><div class="sy-packet">${esc(p.text)}</div><details data-persona-directives-box><summary>强调</summary><p class="sy-help">写在这里的话会单独占档案里的「强调」区：开头一块、结尾再一块，发消息前再单独送一次。自动更新不会删。</p><label>用大白话告诉模型要记住什么<textarea rows="3" data-persona-directive-ask placeholder="例如：纱月已经答应源以后都会软萌地对待他，不需要源每次提醒。"></textarea></label><button type="button" data-persona-directive-draft>让模型整理成强调</button><p class="sy-help">会用人设接口调用一次模型，结果先放进下面的框，你看过、改过再保存。</p><label>强调内容（一行一句）<textarea rows="4" data-persona-directives>${esc((p.userDirectives??[]).join('\n'))}</textarea></label><button type="button" data-persona-directives-save>保存强调</button></details><details data-persona-editor><summary>修改整份人物档案</summary><textarea rows="12" data-persona-edit>${esc(editorText)}</textarea><label>别称与简称（逗号分隔）<input data-persona-aliases value="${esc((p.aliases??[]).join('，'))}"></label><p class="sy-help">简繁自动识别；同一别称对应多人时不自动选人。召回字典中的手工校正优先。</p><label><input type="checkbox" data-persona-lock ${p.locked?'checked':''}>锁定，不让 AI 覆盖</label><button type="button" data-persona-commit>保存人物</button></details>${mergeEditor}${personaSourceDetails(p,d.worldbook?.guide,[...new Map((p.bindings??[]).map(b=>[JSON.stringify([b.book,b.uid]),b])).values()])}<div class="sy-actions"><button type="button" data-persona-undo>恢复上一版并锁定</button><button type="button" data-persona-delete>删除档案</button></div>`;
      ...
      action('[data-persona-directives-save]',async()=>{await app.editDynamicPersona(p.id,{userDirectives:card.querySelector('[data-persona-directives]').value.split(/\n/u)});stamp='';paint(app.state);});
      action('[data-persona-directive-draft]',async()=>{const result=await app.draftDynamicPersonaDirectives(p.id,card.querySelector('[data-persona-directive-ask]').value);card.querySelector('[data-persona-directives]').value=result.directives.join('\n');});
