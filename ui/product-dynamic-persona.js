...
<p class="sy-help">同一人物按阶段保存；修改整份档案，属性可自由增删。锁定后 AI 不覆盖。原世界书不永久停用；可识别的静态人设或安全 EJS 当前阶段在本轮请求替换，复杂脚本保留原样。检查读取、同步镜像不调用模型。</p>
...
export function mountDynamicPersona({panel,app,run,host=globalThis}){
  const $=s=>panel.querySelector(s),root=$('[data-view="dynamic-persona"]'),drafts=new Map();let stamp='',scope='',page=0,profilePage=0,startDirty=false,promptDirty=false,manualDirty=false,manualPage=0,manualStamp='',previewStamp='';
  ...
  bind('[data-persona-budget-save]',async()=>{const patch={dynamicPersonaPrompt:$('[data-persona-prompt]').value};for(const key of ['dynamicPersonaInputUnits','dynamicPersonaOutputTokens','dynamicPersonaDeadlineMs'])patch[key]=Number($('[data-setting="'+key+'"]').value);await app.saveSettings(patch);promptDirty=false;});
