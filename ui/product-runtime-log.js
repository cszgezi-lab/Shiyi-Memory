...
export function mountRuntimeLog({panel,app,run,host,download}){
  ...
  const showText=(data,message)=>{const fallback=$('[data-log-fallback]');const area=$('[data-log-text-value]');area.value=JSON.stringify(data,null,2);fallback.hidden=false;fallback.open=true;status(`${message}（共 ${area.value.length} 字，可长按选择或点复制）`);};
  ...
  // gives a small, complete snapshot of just the failing records.
  $('[data-log-issues-text]').addEventListener('click',async()=>{try{showText(await app.exportRuntimeLog({filter:{levels:['error','warning']}}),'仅失败与提醒记录；可直接复制，不含其余正常日志。');}catch(error){report(error);status(`失败记录未读取：${failureText(error)}`);}});
  ...
  $('[data-log-export]').addEventListener('click',async()=>{
    ...
    try{snapshot=await app.exportRuntimeLog();const result=await download(snapshot,'拾忆-运行日志.json');status(result?.saved===true?`日志已保存到${result.saveLocation??'文件'}${result.exportAttempt==='picker'?'（系统直存失败，已改用文件选择器）':''}。`:result?.status==='dispatched'?'已交给系统下载，但本机无法确认文件已落盘；请确认 Downloads 中出现文件，否则点“查看／复制文本”自行保存。':'日志导出状态未确认；可点“查看／复制文本”直接保存。');}
