...
function detailsHTML(entry){
  ...
  if(d.storageArtifact)lines.push(['存档类型',({vector_jobs:'向量续传检查点',vector_index:'已完成向量索引',vector_staging:'重建中的暂存索引'})[d.storageArtifact]??'未识别']);
...
export function mountRuntimeLog({panel,app,run,host,download}){
  ...
  function paint(next){
    ...
    const selected=[...data.entries].reverse().filter(e=>filter==='all'||filter==='issues'&&['error','warning'].includes(e.level)||filter==='vectors'&&e.task==='vectors'||filter==='summary'&&e.task==='summary'||filter==='merge'&&e.task==='merge'||filter==='api'&&['connection','models','assistant'].includes(e.task));
