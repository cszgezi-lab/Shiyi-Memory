...
export function initProductShell({documentRef=globalThis.document,host=globalThis,application=null,controller=null,controllerOptions={}}={}){
 ...
 const modelRequests=new Map(),dirtyApi=new Set();let autoStartDirty=false,autoScope=null,personEditor=null,knowledgeView=null;let management=null,customManagement=null,logView=null,dictionaryView=null,recallView=null,mergeView=null;let floating=null,lastFeedbackId=null,noticeText='打开聊天后自动加载对应记忆和总结批次。',noticeLevel='info';
 ...
 function paintCards(){
   ...
   $('[data-cards]').innerHTML=memoryListHTML(snapshot.cards,{settings:snapshot.settings,q,category,opened});
   for(const b of $$('[data-hide]'))b.addEventListener('click',()=>run(()=>app.hideRecord(b.dataset.hide)));
   for(const b of $$('[data-edit]'))b.addEventListener('click',()=>management?.edit(b.dataset.edit));
   for(const b of $$('[data-edit-profile]'))b.addEventListener('click',()=>personEditor?.open(b.dataset.editProfile));personEditor?.reattach();
   for(const b of $$('[data-delete]'))b.addEventListener('click',()=>run(async()=>{if(host.confirm?.('删除这条记忆？可从回收站恢复，不影响聊天原文。'))await app.deleteRecord(b.dataset.delete);}));
   for(const b of $$('[data-delete-profile]'))b.addEventListener('click',()=>run(async()=>{
 ...
 function paint(s){snapshot=s;for(const [sel,value]of [['[data-status]',s.message],['[data-scope]',s.chatReady?`当前聊天 · 已连接${s.enabled?'':' · 自动任务未启用'}`:s.status==='loading'?'正在加载当前聊天…':s.status==='no_chat'?'尚未打开聊天':s.stale?'正在重新核对聊天来源…':'等待当前聊天就绪'],['[data-progress]',s.progress],['[data-model]',s.settings.assistantFollowSummary?s.settings.providerModel:s.settings.assistantModel]])if($(sel))$(sel).textContent=value;paintCards();management?.paint(s);customManagement?.paint(s);logView?.paint(s);dictionaryView?.paint(s);recallView?.paint(s);mergeView?.paint(s);
   ...
   for(const f of $$('[data-key]')){const kind=f.getAttribute('data-key');f.placeholder=s.credentialPresent?.[kind]?'已保存 / 已填入；输入新 Key 可替换':'无需认证的服务可以留空';const status=$(`[data-key-status="${kind}"]`);if(status)status.textContent=s.credentialErrors?.[kind]??(s.credentialDirty?.[kind]?'新输入的 Key 尚未保存。':s.credentialSaved?.[kind]?(s.credentialPresent?.[kind]?'Key 已保存，重启后自动使用。':'已保存的 Key 属于其他地址；请重新输入本地址的 Key。'):(s.credentialPresent?.[kind]?'Key 尚未保存，点击本模型的保存按钮。':'尚未保存 Key；无需认证的服务可以留空。'));}
 ...
 function fill({apiOnly=false}={}){const s=app.state;for(const f of $$('[data-setting]')){const key=f.getAttribute('data-setting');if(dirtyApi.has(key))continue;if(f.type==='checkbox')f.checked=Boolean(s.settings[key]);else f.value=s.settings[key]??'';}{if($('[data-input]'))$('[data-input]').value=s.draft??'';if($('[data-count]'))$('[data-count]').value=s.settings.messageCount;if($('[data-batch-size]'))$('[data-batch-size]').value=s.settings.summaryBatchSize;const select=$('[data-conversation]');if(select){select.innerHTML=s.conversations.map(c=>`<option value="${esc(c.id)}">${esc(c.title)}</option>`).join('')||'<option value="main">配置对话</option>';select.value=s.conversationId;}}syncInherited();syncSummaryRange();}
 ...
 const saveAutomatic=async()=>{const floor=Number($('[data-auto-start]').value),scope=app.state.core?.scope,patch=collect($('[data-record-panel="automatic"]'));await app.saveSettings(patch);await app.setAutoStartFloor(floor,scope);autoStartDirty=false;};
 ...
 function showRecordTab(name){for(const section of $$('[data-record-panel]'))section.hidden=section.dataset.recordPanel!==name;for(const tab of $$('[data-record-tab]'))tab.classList.toggle('active',tab.dataset.recordTab===name);setPage('recording');if(name==='logs')void app.loadRuntimeLog?.().then(()=>paint(app.state));}
 ...
 for(const f of $$('[data-key]'))f.addEventListener?.('input',()=>{const kind=f.getAttribute('data-key');app.setKey(kind,f.value,$(`[data-setting="${API_INFO[kind].prefix}Endpoint"]`)?.value);resetModels(kind);if(kind==='summary')resetModels('assistant');});
 ...
 $('[data-search]')?.addEventListener?.('input',paintCards);$('[data-category]')?.addEventListener?.('change',paintCards);$('[data-conversation]')?.addEventListener?.('change',()=>run(async()=>{await app.selectConversation($('[data-conversation]').value);fill();}));
 ...
 mergeView=mountMergeManagement({panel,app,run,host});recallView=mountRecallView({panel,app,run,setPage,host,download});logView=mountRuntimeLog({panel,app,run,host,download});management=mountMemoryManagement({panel,app,run,host});dictionaryView=mountDictionary({panel,app,run,save:async input=>{if(dirtyApi.has('aliases'))throw new Error('手动字典文本尚未保存，请先保存世界设置');await app.saveDictionaryEntry(input);$('[data-setting="aliases"]').value=app.state.settings.aliases;}});customManagement=mountCustomModules({panel,app,run,host,download,onAssistant:async text=>{await app.setDraft(text);fill();setPage('assistant');}});fill();paint(app.state);
 personEditor=mountPersonEditor({panel,app,run});
 knowledgeView=mountKnowledgeView({panel,app,run,host});knowledgeView.paint(app.state);
 floating=mountFloatingProduct({panel,documentRef,host,version:PRODUCT_VERSION,onOpen:()=>{void app.followCurrentChat?.().catch(e=>feedback(`聊天记忆读取未完成：${failureText(e)}`,'warning'));},onStop:()=>run(()=>app.stop()),onLogs:()=>{setPage('recording');showRecordTab('logs');}});floating.setNotice(noticeText,noticeLevel);
 let destroyed=false;
 Promise.resolve(app.loadApiSettings?.()).then(()=>{if(!destroyed){fill({apiOnly:true});paint(app.state);return app.startChatTracking?.();}}).catch(e=>{if(!destroyed)feedback(`读取全局配置失败：${failureText(e)}`,'error');});
 return {panel,application:app,controller:controller??app.core,setPage,floating,
   async destroy(){destroyed=true;clearTimeout(timer);resetAllModels();floating.destroy();await app.dispose?.();}};
}
