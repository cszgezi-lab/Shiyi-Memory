...
export function mountRecallView({panel,app,run,setPage,host=globalThis}){
  ...
  async function loadRows(){
    ...
    try{const result=await app.listVectorEntries({query:$('[data-vector-search]').value,status:$('[data-vector-filter]').value,page:rowPage});if(version!==rowVersion)return;rowPage=result.page;
      $('[data-vector-entries]').innerHTML=result.rows.map(row=>`<div class="sy-info-row"><span>${({indexed:'已索引',stale:'待更新',missing:'未索引'})[row.status]}</span><strong>${esc(row.title)}</strong></div>`).join('')||'<p class="sy-help">没有符合条件的记忆。</p>';
