...
export const LOG_TASKS = Object.freeze({ operation:'插件操作',transport:'API 请求',background:'后台任务',storage:'存档', 'knowledge-vectors':'知识库向量',summary:'总结', merge:'事件合并', connection:'连接测试', models:'拉取模型', assistant:'配置助手', knowledge:'知识库分析', vectors:'建立向量索引', import:'导入文件', recall:'召回预览' });
export const LOG_PHASES = Object.freeze({ retry_wait:'等待并重试临时错误',response_saved:'模型结果已暂存',resume_response:'复用已返回结果',resume_commit:'确认上次已提交',checkpoint_warning:'进度记录待恢复',partial_repair:'补齐缺失内容',start:'任务开始', vector_storage_failed:'向量本机保存失败', vector_failed:'向量请求失败', vector_split:'拆小向量批次重试', vector_commit:'保存向量检查点', range:'读取楼层', request:'发送模型请求', response:'收到模型响应', merge_deferred:'总结先保存，合并单独处理', merge_accepted:'确认同一次事件', merge_separate:'暂不合并', normalize:'兼容模型字段', repair_request:'正在自动纠正字段', repair_response:'收到字段纠错结果', repair_complete:'字段纠错通过', repair_failed:'字段纠错未通过', repair_skipped:'跳过字段纠错', floors:'检查逐楼摘要', validate:'校验记忆内容', commit:'保存记忆', complete:'任务完成', failed:'任务失败', canceled:'任务停止' });
...
export function safeLogDetails(value = {}) {
  ...
  if (['vector_jobs','vector_index','vector_staging','runtime_log','workspace','memory','checkpoint','credentials','settings'].includes(value?.storageArtifact)) result.storageArtifact=value.storageArtifact;
  return result;
}
