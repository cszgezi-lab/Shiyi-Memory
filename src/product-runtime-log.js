...
export const LOG_PHASES = Object.freeze({ plan:'本批请求计划',wait_complete:'等待结束', retry_wait:'等待并重试临时错误',stage_start:'开始分工阶段',stage_complete:'分工阶段已暂存',stage_resume:'复用已完成阶段',quality_split:'拆小校对范围',recovery_compact:'失败后压缩可选上下文',response_saved:'模型结果已暂存',resume_response:'复用已返回结果',resume_commit:'确认上次已提交',checkpoint_warning:'进度记录待恢复',partial_repair:'补齐缺失内容',start:'任务开始', vector_storage_failed:'向量本机保存失败', vector_failed:'向量请求失败', vector_split:'拆小向量批次重试', vector_commit:'保存向量检查点', range:'读取楼层', request:'发送模型请求', response:'收到模型响应', merge_deferred:'总结先保存，合并单独处理', merge_accepted:'确认同一次事件', merge_separate:'暂不合并', normalize:'兼容模型字段', repair_request:'正在自动纠正字段', repair_response:'收到字段纠错结果', repair_complete:'字段纠错通过', repair_failed:'字段纠错未通过', repair_skipped:'跳过字段纠错', floors:'检查逐楼摘要', validate:'校验记忆内容', commit:'保存记忆', complete:'任务完成', failed:'任务失败', canceled:'任务停止' });
