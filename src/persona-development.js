...
export const PERSONA_IMPACT_RULE=`动态人设不是第二份事件记忆。客观事件、完整时间线和发生经过由记忆模块保存；这里只写现在仍有效的态度、边界和表达：对谁、在何种场合、现在怎样。当前描述与development按对象写现在状态，重大变化放最前；同一对象、同一情境只保留当前状态，不逐批追加日记。被后续状态代替的旧说法退出当前描述，不改写成留在当前层的否定句。客观事实留在底稿。转变原因只留一句与楼号，不重复事件经过。
