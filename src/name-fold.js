...
export function nameMentionAt(text,word,at){
  ...
  return (!before||/[\s，。！？、：；“”‘’「」『』（）()《》【】,.:;!?"']$/u.test(before)||/(?:看向|对|叫|喊|问)$/.test(before))&&(!after||/^[\s，。！？、：；“”‘’「」『』（）()《》【】,.:;!?"']/u.test(after)||/^(?:说|问|回答|笑|看|走|来|去|点头|摇头|想|将|接|拿|是|在|有|没有|没|不|也|又|正|仍|很|的|和|与|对|把|被|让|给|会|能|今天|现在|这时|刚|已经|以前|喜欢|讨厌|吃|喝|听|穿|坐|站|躺|回|抬|低|伸|拉|抱|哭|睡|醒)/.test(after));
}
