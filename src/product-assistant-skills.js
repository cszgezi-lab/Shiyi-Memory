...
export const ASSISTANT_SKILL_VERSION='1.0';
export const ASSISTANT_SKILLS=Object.freeze([
  ...
  {id:'summary',title:'分批总结、完整事件与合并',text:'一条消息是一楼，按 summaryBatchSize 分批。完整事件保留起因、过程、结果、参与者、时间、来源及重要原句；召回速览独立，不把完整纪要压成一句话。同一事件应链接已有事件编号合并，主题相近不是同一事件。发生时间可兼容才合并；无法解析一致时独立保存并生成核对项，明确冲突不能强并。原文未给出的知情者不能由参与者名单直接推定。重做同范围批次只在成功后替代旧结果，失败不清空旧记忆。使用 recordingRules 指定关注点； inputBudgetUnits 是输入估算，outputBudgetUnits 是回复 Token 上限，0 沿用服务默认，二者不是同一项。'},
  {id:'recall',title:'字典、标签、向量与轻量注入',text:'dictionaryEnabled 和 tagRecallEnabled 默认开启。总结中的有来源名称/别称与标签形成字典，保存后更新。aliases 中 主名=别名 | 关联检索词；! 前缀停用，!! 前缀删除且阻止自动恢复。语义相近不意味着同一个人。BM25、字典扩展、标签和分类候选先融合，再按启用情况重排；默认参数见 settings。向量是派生索引，需配置向量服务并开启 vectorEnabled；vectorAutoUpdate 在新增/修改后后台增量更新，不在每轮重建。排除索引不删除原记忆。注入仅相关事件速览和查询相关片段，不整批复制全部纪要。不要用很短的总超时伪装高效，保留在线降级提示。'},
