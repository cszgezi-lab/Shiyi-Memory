...
export const ASSISTANT_SKILL_VERSION='1.3';
export const ASSISTANT_SKILLS=Object.freeze([
  ...
  {id:'summary',title:'分批总结、完整事件与合并',text:'一条消息是一楼，按 summaryBatchSize 分批。记录→总结批次的“继续未完成”和“重新生成”不同：继续复用冻结任务、已返回结果与已提交子批；重新生成才重做整批。可继续全部未完成或所选未完成，成功批次跳过。暂存只有本机、按聊天隔离，不在日志和备份里，续跑有效期7天、使用时淘汰过期暂存，每个聊天最多24份/32MB、单份8MB；不能把暂存当正式记忆。旧版本无暂存时需重新生成，原文或记录规则变化时不能复用旧草稿。正常每个内部子批一次总结，缺楼/补充区块缺字段时只补缺失部分；临时网络错误和补全共享每批最多2次额外请求，不无限重试。来源、事件关联、时间与知情边界校验不通过不能强行保存。完整事件保留起因、过程、结果、参与者、时间、来源及重要原句；召回速览独立，不把完整纪要压成一句话。同一事件应链接已有事件编号合并，主题相近不是同一事件。跨批合并与总结分开：来源合格的总结先保存；autoMergeEnabled 默认开启，随后用总结模型逐对核对，每次最多10对。记录→事件合并可单独重试、改目标、保持独立或撤销；合并失败不重跑总结。自动合并只处理总结模型提出的候选，不会扫描全部记忆猜测相似项。知情范围随原片段保留；时间明确冲突不能强并。合并是可撤销视图，原批次删除/重生或内容修改使旧关系失效。原文未给出的知情者不能由参与者名单直接推定。重做同范围批次只在成功后替代旧结果，失败不清空旧记忆。使用 recordingRules 指定关注点； inputBudgetUnits 是输入估算，outputBudgetUnits 是回复 Token 上限，0 沿用服务默认，二者不是同一项。'},
  {id:'recall',title:'字典、标签、向量与轻量注入',text:'dictionaryEnabled 和 tagRecallEnabled 默认开启。总结中的有来源名称/别称与标签形成字典，保存后更新。aliases 中 主名=别名 | 关联检索词；! 前缀停用，!! 前缀删除且阻止自动恢复。语义相近不意味着同一个人。BM25、字典扩展、标签和分类候选先融合，再按启用情况重排；默认参数见 settings。向量是派生索引，需配置向量服务并开启 vectorEnabled；vectorAutoUpdate 在新增/修改后后台增量更新，不在每轮重建。排除索引不删除原记忆。召回→向量提供“重试全部未完成”和单条重试，只调用向量 API，不重做总结；成功索引和长文本分段进度保留。向量返回后先保存独立的单次响应检查点；随后进度文件或索引发布失败，可复用已确认写入的响应，不重做总结。磁盘完全不可写时不能保证重启恢复，不能声称不会丢失尚未落盘的结果。网络或认证错误暂停后台队列，修复配置后手动重试。TT 2.2 向量使用 WebView 直连，模型列表成功不代表向量一定可达。注入仅相关事件速览和查询相关片段，不整批复制全部纪要。不要用很短的总超时伪装高效，保留在线降级提示。'},
