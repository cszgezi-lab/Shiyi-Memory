...
export function moduleSummaryContract(legacy) {
  return {
    ...
    currentStateRules:'关系和约定不按每批重复抄写。无变化不新增；有变化记录这次变化及当前态、对象和适用条件，不再堆一份事件流水账。只有本批明确接续relevantRecords中同一事项时，才逐字引用其id：约定用completionOf（履行）或supersedes（接受/取消等新状态），关系用supersedes。保留旧记录的对象、条件、temporal.plannedFor以界定同一约定；改期必须先取消旧约，再另建新时间的约定。不同事项、不同对象、临时情绪、回忆旧情节不互相覆盖，禁止猜id。新记录应完整表达当前状态及仍有效的边界，不用旧记录的来源冒充本批证据；历史由程序保留。',
    ...
    fields:{
      ...
      relationshipChanges:['sourceId or sourceRefs','from(关系主体的人名，不是变化前状态)','to(关系对象的人名，不是变化后状态)','description(这两人之间怎样变化，保留旧边界与本次确认)','evidenceKind','epistemicStatus','supersedes(optional)','context(optional)','scope(optional)','keyDialogues(optional,有关键原话时提取)','viewpoints(optional,有观念证据时提取)'],
      ...
      commitmentChanges:['sourceId or sourceRefs','participants','content','state','temporal(optional: plannedFor/actualAt)','epistemicStatus','completionOf(optional)','supersedes(optional)','context(optional)','conditions(optional)'],
